...
if(server) {
    ...
    mobjs.forEach((m) => {
      if(m.addedNodes) {
        m.addedNodes.forEach((el) => {
          if(el.nodeType === 1) {
            isct.setIsctEl(el)
            const subNodes = el.querySelectorAll('*')
            subNodes.forEach(subEl => {
              if(subEl.nodeType === 1){
                isct.setIsctEl(subEl)
              }
            })
          }
        })
  ...
  ws.onopen = function () {
    ...
    window.addEventListener('scroll', evt => {
      ...
      if(evt.isTrusted && !ws.isctEvent) {
        sendMessage({type: 'windowScroll', x, y, w, h})
      }
      if(ws.isctEvent){
        ws.isctEvent = false
      }
    ...
    document.documentElement.addEventListener('focus', evt => {
      const target = evt.target.dataset.isctId,
            type = evt.type
    ...
    document.documentElement.addEventListener('blur', evt => {
      const target = evt.target.dataset.isctId,
            type = evt.type
    ...
    document.documentElement.addEventListener('change', evt => {
      ...
      // 这里可能会有一点问题，会导致 checkbox 的 change 执行两次
      // 但是不这么写又会导致 input 的 change 不执行
      if(evt.isTrusted) {
        sendMessage({type, target})
      }      
    })
